...
const setWeezeventSrc = () => {
	...
	iframe.src =
		`https://widget.weezevent.com/ticket/E771096/?code=36089&locale=${locale}&width_auto=1&color_primary=FFD6AC&v=2` +
		iframe.dataset.key;
};
	...
	titles.forEach((title, index) => {
		if (window.innerWidth <= 780) {
			title.addEventListener("click", (e) => {
				...
				translateTitles(
					index,
					Array.from(titles).filter((t) => t !== title)
				);
			});
		} else {
			title.addEventListener("click", (e) => {
...
const initializeAccordions = () => {
	accordions.forEach((accordion) => {
		accordion.addEventListener("click", (e) => {
			let activeAccordion = Array.from(
				document.querySelectorAll(".accordion-item h4")
			).find((accordion) => accordion.parentNode.classList.contains("active"));
...
const displayContent = (titleName) => {
	...
	if (
		...
		titleName !== "menu" &&
		titleName !== "les-ami-e-s-d-eros"
	) {
		titleToDisplay = document.getElementById(titleName).previousElementSibling;
		menuIndex = Array.from(titles).findIndex(
			(title) => title === titleToDisplay
		);
		...
		document.querySelector(".weezevent2021").style.display = "block";
	}
};
