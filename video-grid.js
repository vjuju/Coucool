...
function setImg(time) {
  ...
  images.forEach((img) => {
    ...
    img.src = `https://cou.cool/2021/dance_images/dance_${img_nb_str}.jpg`;
...
function onAlmostImageClick(event) {
  ...
  if (isPlaying) {
    ...
    almostImage.src = `https://cou.cool/2021/dance_images/dance_${img_nb_str}.jpg`;
    showMessage("almost");
  }
}
...
function showMessage(message_type) {
  pauseVideo(
    document.querySelector(".playOrPause"),
...
function launchControls() {
  ...
  function playPauseMedia() {
    ...
    if (isPlaying) {
      ...
      setTimeout(function () {
        document
          .querySelectorAll(".explanations")
          .forEach((explanation) => (explanation.style.display = "none"));
      }, 500);
      explanations.forEach((explanation) =>
  ...
  function toggleFwdOrRwd() {
    if (
      fwdOrRwd.textContent == "Backward" ||
      fwdOrRwd.textContent == "A l'envers"
    ) {
      fwdOrRwd.textContent =
        document.documentElement.getAttribute("lang") === "en"
          ? "Forward"
          : "A l'endroit";
    } else {
...
function resizeVideoGridContainer() {
  let videoWidth = parseInt(
    getComputedStyle(document.documentElement).getPropertyValue(
      "--video-initial-width"
    )
  );
  let videoHeight = parseInt(
    getComputedStyle(document.documentElement).getPropertyValue(
      "--video-initial-height"
    )
  );
  let footerAndHeaderHeight = parseInt(
    getComputedStyle(document.documentElement).getPropertyValue(
  ...
  let mainRatio =
    window.innerWidth / (window.innerHeight - parseInt(footerAndHeaderHeight));
  let videoGridContainer = document.querySelector("#video-grid-container");
  if (videoRatio > mainRatio) {
    videoGridContainer.classList.replace(
      "overflowing-height",
      "overflowing-width"
    );
  } else {
